...
# echo "ssh-ing publicly"
# ssh hank@hank.hopto.org && "Success " && exit 0 

myIp=$(ip addr | awk '/192./ {print $2}' | cut -d'/' -f1- | head -1)
echo "Current Ip = $myIp"

[ -z "$myIp" ] && echo -e "Not connected to a network\nExiting.." && exit 1

subnet=$(echo "$myIp" | cut -d '.' -f1-3).0/24
echo "Subnet = $subnet"

echo "Scanning Available Ips in the subnet having ssh open..."
availableIps=$(nmap -p 22 --open -sV $subnet | grep 192 | cut -d ' ' -f 5 | tr ' ' '\n')
echo $availableIps
...
echo "Probable raspberry pi Ip = $raspiIp"
